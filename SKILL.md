...
8. **苏格拉底质询** → 加载 [rules/socratic.md](rules/socratic.md) 执行
   - 7 步流程：AI 自审(1) → 证据构建(2) → 亮点确认(3) → 攻击性质询 3 轮(4) → 触发条件提炼(5) → 学习路径推荐(6) → 输出(7)
   - Step 4/5/6 使用 AskUserQuestion 提供结构化回复选项
...
3. 苏格拉底质询 → 加载 rules/socratic.md 执行
   ...
   - Step 5：触发条件提炼（含 Gene 验证中需修订的资产）
   - 质询完成后追加「思维尸检」+「行为纠偏」+「Gene 候选表」到报告
...
```
前置条件：用户已确认报告内容（包括苏格拉底质询 Step 5 的 Gene 候选）

执行：加载 rules/gene-protocol.md 执行 Step 6.1 ~ 6.5

具体流程：
1. 收集 Gene 候选（来自质询 Step 5 + 报告改进建议）
2. 分类为 gene/sop/pref
3. 写入 memory/：
